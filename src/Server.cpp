    ...
    m_pidFileName(pidFileName)
{
	...
	{
		struct passwd *pw = getpwnam(Config::Instance()->User.c_str());

		if(pw)
		{
		    //pw->pw_dir
			syslog(LOG_NOTICE, "setting user to %s", Config::Instance()->User.c_str());

			if(setuid(pw->pw_uid)!=0)
            {
                syslog(LOG_ERR,"error setuid to: %s",Config::Instance()->User.c_str());
            }

			if(setgid(pw->pw_gid)!=0)
            {
                syslog(LOG_ERR,"error setgid to: %s",Config::Instance()->Group.c_str());
            }
		}
	}
