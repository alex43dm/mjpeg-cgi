    ...
    for(i = 0; i < ixmlNodeList_length(deviceList); i++)
    {
        ...
        if(serviceList)
        {
            for(j = 0; j < ixmlNodeList_length(serviceList); j++)
            {
                ...
                url = xml_getChildElementValue(serviceElement, "SCPDURL");
                ...
                {
                    s->SCPDUrl = _baseURL + url;
                }

                url = xml_getChildElementValue(serviceElement, "eventSubURL");
                if(url.find("http") != std::string::npos)
                {
                    s->eventSubURL = url;
                }
                ...
                {
                    s->eventSubURL = _baseURL + url;
                }

                url = xml_getChildElementValue(serviceElement, "controlURL");
                if(url.find("http") != std::string::npos)
                {
                    s->controlURL = url;
                }
...
IXML_Document *Device::SendAction(ServiceType stype, const char* _actionName, int count, ...)
{
    ...
    if(res)
    {
        Log::err("Unable to send action %s(code: %d error: %s url: %s DevUDN: %s)",
                 actionName, ret, UpnpGetErrorMessage(ret), url, urn);
    }
